...
import { scrapeWebPage } from "../../tools/webScraper.js";

const tools = [scrapeWebPage];
...
async function callModel(state) {
  const target = state.company || state.task;
  const systemMsg = {
    role: "system",
    content: `You are a web research scout. Use the scrape_web_page tool to fetch and extract content from a relevant page (e.g., product/pricing) for: ${target}. After scraping, summarise key product/pricing info.`,
  };
  const priorMsgs = messagesOf(state);
  const humanTurn =
    priorMsgs.length > 0
      ? priorMsgs
      : [{ role: "user", content: `Begin web research for ${target}.` }];
...
async function writeOutput(state) {
  const msgs = messagesOf(state);
  const lastMsg = msgs[msgs.length - 1];
  const content = lastMsg?.content || "No web data obtained.";
  return { messages: msgs, web_data: content };
}

const toolNode = new ToolNode(tools);

const webScoutGraph = new StateGraph(GraphAnnotation)
